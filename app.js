...
window.OneSignalDeferred.push(async function(OneSignal) {
  try {
    await OneSignal.init({
      ...
      serviceWorkerPath: "ALL4U/onesignal/OneSignalSDKWorker.js",
      serviceWorkerParam: { scope: "/ALL4U/onesignal/" },
    ...
    // Supabase anonymous user becomes the OneSignal External ID.
    // This lets the server target Tavşan or Mirket without exposing a secret key.
    if (currentUser?.id) {
      try { await OneSignal.login(currentUser.id); } catch (e) { console.warn("OneSignal login", e); }
    }
  } catch (e) {
...
async function enableRealPush() {
  if (!oneSignalReady) {
    return showToast("Bildirim sistemi hazırlanıyor, birkaç saniye sonra tekrar dene");
  }
  window.OneSignalDeferred.push(async function(OneSignal) {
    try {
      if (currentUser?.id) await OneSignal.login(currentUser.id);
      await OneSignal.Notifications.requestPermission();
      const enabled = OneSignal.Notifications.permission;
      showToast(enabled ? "Gerçek bildirimler açıldı ♡" : "Bildirim izni verilmedi");
    } catch (e) {
      console.warn(e);
...
// Replace the old notification settings action with OneSignal permission.
...
const oldBootCloudV7 = bootCloud;
bootCloud = async function() {
  await oldBootCloudV7();
  if (currentUser?.id) {
    window.OneSignalDeferred = window.OneSignalDeferred || [];
    window.OneSignalDeferred.push(async function(OneSignal) {
      try { await OneSignal.login(currentUser.id); } catch (e) { console.warn("OneSignal login", e); }
